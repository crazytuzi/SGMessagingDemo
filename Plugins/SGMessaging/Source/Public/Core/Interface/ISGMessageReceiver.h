 ...
 * a call to its ISGMessageReceiver.ReceiveMessage method for each message that was sent directly to it (via ISGMessageBus.Send)
 * and for each published message (via ISGMessageBus.Publish) that it subscribed to (via ISGMessageBus.Subscribe).
 *
 * This interface provides a rather low-level mechanism for receiving messages. Instead of implementing it, Most users will
 * want to use an instance of see FSGMessageEndpoint, which provides a much more convenient way of sending and receiving messages.
 *
 * @see FSGMessageEndpoint, ISGMessageBus, ISGMessageSender
 */
