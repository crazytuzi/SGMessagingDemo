 ...
 * (ISGMessageContext) that contain additional information about a message, such as when it was sent and the sender and recipients.
 *
 ...
 * using so called Message Scopes (ESGMessageScope).
 *
 * All message recipients (objects implementing the see ISGMessageReceiver interface) must be registered with the message bus through the
 * see ISGMessageBus.Register method. Before a recipient is destroyed it should unregister itself using the see ISGMessageBus.Unregister
 * method. Message senders (objects implementing the see ISGMessageSender interface) do not register with the bus, but instead pass a
 * reference to themselves each time they send a message. The ISGMessageReceiver and ISGMessageSender both are very low-level interfaces
 * into the messaging system. Most users will prefer to use instances of the see FSGMessageEndpoint class instead, which provides a much
 ...
 *
 * In the Request-Reply pattern a message is sent to one or more particular message recipients using the ISGMessageBus.Send method.
 * Message recipients implement the see ISGMessageRecipient interface and are uniquely identified by their addresses (FSGMessageAddress).
 * After a message is received, the recipients may then reply with another message using the same ISGMessageBus.Send method. Alternatively,
 * a previously received message may be forwarded to another recipient using the see ISGMessageBus.Forward method. This pattern is useful
 ...
 *
 * In the Publish-Subscribe pattern a message is sent to all message recipients on the bus using the see ISGMessageBus.Publish method.
 * Only recipients that previously subscribed to the type of the sent message using the ISGMessageBus.Subscribe method will actually receive
 ...
 * another message, either directly to the message sender using ISGMessageBus.Send method, or by publishing another message using the
 * the ISGMessageBus.Publish method. This pattern is useful for discovering recipients on the bus and to message recipients that are unknown.
 *
 ...
 * implementing the ISGMessageInterceptor interface) with the message bus. This feature enables advanced use cases that require inspection or
 ...
 * implementing the see ISGMessageTracer interface) that can be accessed with the ISGMessageBus.GetTracer() method. The message tracer is
 ...
 *
 * @see FSGMessageAddress, FSGMessageEndpoint, ISGMessageReceiver, ISGMessageSender
 */
class ISGMessageBus
{
public:
	 ...
	 */
	virtual TSharedRef<ISGMessageTracer, ESPMode::ThreadSafe> GetTracer() = 0;
	 ...
	 * @see Register
	 */
	virtual void Unregister(const FSGMessageAddress& Address) = 0;
	 ...
	 */
	virtual void AddNotificationListener(const TSharedRef<ISGBusListener, ESPMode::ThreadSafe>& Listener) = 0;

	/**
	 * Remove a listener to the bus notifications
	 * @param Listener The listener to remove from the registration notifications
	 */
	virtual void RemoveNotificationListener(const TSharedRef<ISGBusListener, ESPMode::ThreadSafe>& Listener) = 0;
